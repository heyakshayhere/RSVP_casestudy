...
-- the count form genere is 14662
SELECT COUNT(*) FROM ratings;
-- the count form ratings is 7997
SELECT COUNT(*) FROM director_mapping;
-- the count form director_mapping is 3867
SELECT COUNT(*) FROM names;
-- the count form names is 25735
SELECT COUNT(*) FROM role_mapping;
-- the count form role_mapping is 15615
SELECT COUNT(*) FROM movie;
-- the count form movie is 7997
...
SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) language, 
SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) production_company 
FROM movie;

-- country,worlwide_gross_income,language,production_company has null count 
...
-- in 2019 USA and India produed 1059 movies 
...
-- so drama has the heighest number of movies and has count = 4285
...
-- the rank of thriller movie is 3 with 1484 movies
...
FROM ratings;
-- the name which were given into the table has mistake in the title 
-- it is further corrected in the code    
...
-- Type your code below:
-- It's ok if RANK() or DENSE_RANK() is used too
WITH production_company_hit_movies
AS (
...
/*the null count found is as follows:
height_nulls = 17335
...
AS (
	SELECT genre
		...
		,DENSE_RANK() OVER (
			PARTITION BY year ORDER BY CAST(replace(replace(ifnull(worlwide_gross_income, 0), 'INR', ''), '$', '') AS DECIMAL(10)) DESC
			) AS movie_rank
	FROM movie AS m
	INNER JOIN genre AS g ON m.id = g.movie_id
	WHERE genre IN (
...
FROM production_summary LIMIT 2;
